...
fn test_format_call_args_wrap() {
    let fmt = |src: &str| -> String {
        ...
        format(
            ...
            &case.interner,
            &case.files.get(case.ast.file_id).unwrap().source,
            RendererConfig { max_line_width: 80, indent_width: 4, trailing_comma: true },
        )
    };
    ...
    assert_eq!(
        fmt("fn f() { host::draw_rect(food_x * CELL_SIZE, food_y * CELL_SIZE, CELL_SIZE, CELL_SIZE, 0xFFFFFF00); }"),
        indoc! {"
            fn f() {
                host::draw_rect(
                    food_x * CELL_SIZE,
    ...
    assert_eq!(
        fmt("fn f() { obj.render(food_x * CELL_SIZE, food_y * CELL_SIZE, CELL_SIZE, CELL_SIZE, 0xFFFFFF00); }"),
        indoc! {"
            fn f() {
                obj.render(
                    food_x * CELL_SIZE,
...
fn test_format_local_definition_wraps() {
    ...
    assert_eq!(
        fmt("memory heap: Memory<Size = u32>; fn set_pixel(x: u32, y: u32) { local base: heap::*mut u8 = (fb_ptr() + (y * FB_WIDTH + x) * 3) as heap::*mut u8; }"),
        indoc! {"
    ...
    assert_eq!(
        fmt("fn f() { local p = Point::{ x: very_long_name_one, y: very_long_name_two, z: very_long_name_three }; }"),
        indoc! {"
            fn f() {
                local p = Point::{
...
fn test_format_inline_blocks() {
    ...
    assert_eq!(
        fmt("memory heap: Memory<Size = u32>; fn check(data: heap::[]u8) -> bool { if data.len() < 4 { return false }; true }"),
        indoc! {"
            memory heap: Memory<Size = u32>;
    ...
    assert_eq!(
        fmt("fn f() -> i32 { if some_very_long_condition_variable { return some_very_long_return_value_here } 0 }"),
        indoc! {"
            fn f() -> i32 {
...
fn test_format_binary_chain_breaks_at_line_limit() {
    ...
    assert_eq!(
        fmt("memory heap: Memory<Size = u32>; fn read(data: heap::[]u8, off: u32) -> i32 { (data[off] as i32) | ((data[off + 1] as i32) << 8) | ((data[off + 2] as i32) << 16) | ((data[off + 3] as i32) << 24) }"),
        indoc! {"
            memory heap: Memory<Size = u32>;
