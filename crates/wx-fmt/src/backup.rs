...
impl<'a> Builder<'a> {
    ...
    fn build(&self, ast: &ast::AST) -> Node {
    ...
    fn build_item(&self, item: &ast::Item) -> Node {
        match item {
            ast::Item::Function {
                signature,
                block,
            ...
            ast::Item::Global {
                ...
                value,
                ..
            } => self.build_global_definition(*pub_span, *mut_span, name, type_annotation, value),
            ast::Item::Export { entries } => self.build_export_definition(entries),
            ast::Item::Import {
                module,
                alias,
                entries,
    ...
    fn build_module_definition(
        ...
        if !items.is_empty() {
            nodes.push(Node::Indent(Box::new(Node::Concat(
                vec![Node::HardLine, self.build_item_list(items, false)].into(),
            ))));
            nodes.push(Node::HardLine);
        }
    ...
    fn build_impl_definition(
        ...
        if !items.is_empty() {
            nodes.push(Node::Indent(Box::new(Node::Concat(
                vec![Node::HardLine, self.build_impl_item_list(items)].into(),
            ))));
            nodes.push(Node::HardLine);
        }
    ...
    fn build_impl_item_list(&self, items: &[ast::Separated<ast::Spanned<ast::ImplItem>>]) -> Node {
        let mut nodes = Vec::new();
        for (index, item) in items.iter().enumerate() {
            if index > 0 {
                nodes.push(Node::BlankLine);
    ...
    fn build_impl_item(&self, item: &ast::ImplItem) -> Node {
        match item {
            ast::ImplItem::Method {
                pub_span,
                attributes,
            ...
            ast::ImplItem::Const {
                ...
                let mut nodes = vec![Node::StaticText("const "), self.symbol(name.inner)];
            ...
            ast::ImplItem::AssociatedType { name, ty, .. } => Node::Concat(
                vec![
    ...
    fn build_trait_definition(
        ...
        if !items.is_empty() {
            nodes.push(Node::Indent(Box::new(Node::Concat(
                vec![Node::HardLine, self.build_trait_item_list(items)].into(),
            ))));
            nodes.push(Node::HardLine);
        }
    ...
    fn build_trait_item(&self, item: &ast::TraitItem) -> Node {
        match item {
            ...
            ast::TraitItem::Const { name, ty, .. } => Node::Concat(
                vec![
            ...
            ast::TraitItem::AssociatedType { name, bounds, .. } => {
                let mut nodes = vec![Node::StaticText("type "), self.symbol(name.inner)];
        ...
        if !variants.is_empty() {
            let variant_items = variants
                ...
                .flat_map(|(index, variant)| {
                    let mut variant_nodes = vec![self.symbol(variant.inner.inner.name.inner)];
    ...
    fn build_attributes(&self, out: &mut Vec<Node>, attributes: &[ast::Attribute]) {
    ...
    fn build_type_params(&self, out: &mut Vec<Node>, type_params: &[ast::TypeParam]) {
        if type_params.is_empty() {
            return;
        }
        out.push(Node::StaticText("<"));
    ...
    fn build_function_signature(&self, out: &mut Vec<Node>, signature: &ast::FunctionSignature) {
    ...
    fn build_fn_body(&self, block: &ast::Spanned<ast::Expression>) -> Node {
        match &block.inner {
            ast::Expression::Block { statements } => self.build_block(statements, true),
            ...
            items.push(Node::Indent(Box::new({
                ...
                for (index, statement) in statements.iter().enumerate() {
                    if index > 0 {
                        ...
                        let blank_lines =
                            Self::count_blank_lines(self.source, prev_end as usize, blank_end);
    ...
    fn build_expression(&self, expression: &ast::Spanned<ast::Expression>) -> Node {
        match &expression.inner {
            ...
            ast::Expression::Binary {
                ...
                // In break mode: "a\n    | b\n    | c\n    | d"
                let mut operands: Vec<&ast::Spanned<ast::Expression>> = vec![right, left];
            ...
            ast::Expression::Block { statements } => self.build_block(statements, false),
            ast::Expression::Unreachable => Node::StaticText("unreachable"),
            ...
            ast::Expression::Deref { pointer } => {
                Node::Concat(vec![self.build_expression(pointer), Node::StaticText(".*")].into())
            }
            ...
            ast::Expression::ArrayRepeat { value, count } => Node::Concat(Box::new([
                Node::StaticText("["),
            ...
            ast::Expression::Index { object, index } => Node::Concat(Box::new([
            ...
            ast::Expression::SliceRange { object, start, end } => {
                let mut parts = vec![self.build_expression(object), Node::StaticText("[")];
                if let Some(s) = start {
    ...
    fn build_pattern(out: &mut Vec<Node>, interner: &ast::StringInterner, pattern: &ast::Pattern) {
        match pattern {
            ast::Pattern::Wildcard => out.push(Node::StaticText("_")),
            ...
            ast::Pattern::Struct { name, fields } => {
                ...
                for (i, field) in fields.iter().enumerate() {
                    ...
                    let flen = interner
                        ...
                        .len() as u32;
                    out.push(Node::Symbol {
                        symbol: field.inner.inner.name.inner,
                        len: flen,
                    });
    ...
    fn build_statement(&self, statement: &ast::Statement) -> Node {
        match statement {
            ast::Statement::Expression(expression) => self.build_expression(expression),
    ...
    fn build_path(&self, path: &ast::Path) -> Node {
    ...
    fn build_type_expression(&self, type_expression: &ast::TypeExpression) -> Node {
        match type_expression {
            ast::TypeExpression::Infer => Node::StaticText("_"),
                ...
                if params.len() > 0 {
                    items.push(Node::Indent(Box::new({
                        ...
                        for (index, param) in params.iter().enumerate() {
                            ...
                            param_items
                                .push(self.build_type_expression(&param.inner.inner.ty.inner));
            ...
            ast::TypeExpression::Tuple { elements } => {
                ...
                if !elements.is_empty() {
                    ...
                    let element_items = elements
                        ...
                        .enumerate()
                        .flat_map(|(index, element)| {
                            let mut nodes = vec![self.build_type_expression(&element.inner)];
            ...
            ast::TypeExpression::MemoryTagged { memory, inner } => Node::Concat(
                vec![
