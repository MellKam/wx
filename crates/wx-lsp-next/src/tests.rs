...
fn file_id_for(compiled: &CompiledRoot, path: &Path) -> FileId {
    compiled
        ...
        .unwrap_or_else(|| panic!("file not found in compiled graph: {}", path.display()))
        .file_id
}
...
fn compile_source(root: &PathBuf, source: &str) -> (ServerState, CompiledRoot) {
    let mut state = ServerState::default();
    state.open_documents.insert(root.clone(), open_document(source));
...
fn completion_inside_function_includes_params() {
    ...
    let items = completion_items(&compiled.tir, &compiled.graph.interner, &compiled.symbol_index, file_id, source, cursor);
    let labels: Vec<&str> = items.iter().map(|i| i.label.as_str()).collect();

    assert!(labels.contains(&"a"), "expected param `a` in completions; got: {labels:?}");
    assert!(labels.contains(&"b"), "expected param `b` in completions; got: {labels:?}");
}
...
fn completion_inside_function_includes_locals_declared_before_cursor() {
    ...
    assert!(labels.contains(&"x"), "expected `x` in completions; got: {labels:?}");
    assert!(!labels.contains(&"y"), "expected `y` NOT in completions yet; got: {labels:?}");
}
...
fn completion_prefix_filters_results() {
    ...
    assert!(labels.contains(&"alpha"), "expected `alpha` in completions; got: {labels:?}");
    assert!(!labels.contains(&"beta"), "expected `beta` filtered out; got: {labels:?}");
}
...
fn find_enclosing_function_returns_correct_function() {
    ...
    assert!(first_idx.is_some(), "expected to find enclosing function for cursor in `first`");
    assert!(second_idx.is_some(), "expected to find enclosing function for cursor in `second`");
    assert_ne!(first_idx, second_idx, "cursor positions in different functions should map to different indices");
}
