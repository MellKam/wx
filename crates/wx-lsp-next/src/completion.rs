...
pub fn classify_context(source: &str, offset: usize) -> CompletionContext {
    ...
    let before = source[..prefix_start].trim_end_matches(|c: char| c.is_ascii_whitespace());
    ...
    match before_bytes[before_bytes.len() - 1] {
        b'.' => {
            let before_dot =
                before[..before.len() - 1].trim_end_matches(|c: char| c.is_ascii_whitespace());
            CompletionContext::DotAccess {
                receiver_end: before_dot.len(),
            }
        }
        b':' if before_bytes.len() >= 2 && before_bytes[before_bytes.len() - 2] == b':' => {
            let before_colons =
                before[..before.len() - 2].trim_end_matches(|c: char| c.is_ascii_whitespace());
...
pub fn find_enclosing_function(tir: &TIR, file_id: FileId, cursor_offset: u32) -> Option<usize> {
...
pub fn local_completion_items(
    ...
    let innermost_idx = body
        ...
        .iter()
        .enumerate()
        .filter(|(_, s)| s.span.start <= cursor_offset && s.span.end > cursor_offset)
    ...
    while let Some(scope_idx) = current {
        ...
        for (local_idx, local) in scope.locals.iter().enumerate() {
            ...
            if !is_param && is_innermost && local.name.span.start >= cursor_offset {
                continue;
            }
...
pub fn completion_items(
    ...
    match classify_context(source, offset) {
        CompletionContext::Identifier => {
            ...
            let mut items = match find_enclosing_function(tir, file_id, cursor_offset) {
                Some(func_index) => {
                    local_completion_items(tir, func_index, interner, cursor_offset, prefix)
                }
                None => Vec::new(),
            };

            items.extend(global_completion_items(tir, interner, symbol_index, prefix));
            items
        }
        CompletionContext::DotAccess { .. } => {
...
mod tests {
    ...
    fn single_colon_is_type_annotation() {
        assert_eq!(classify_context("x:", 2), CompletionContext::TypeAnnotation);
    }
    ...
    fn single_colon_not_confused_with_double_colon() {
        ...
        assert_eq!(classify_context("a:", 2), CompletionContext::TypeAnnotation);
    }
