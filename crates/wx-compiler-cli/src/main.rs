...
fn main() {
    let diag_style_arg = clap::Arg::new("diagnostics")
        ...
        .value_parser(clap::builder::PossibleValuesParser::new([
            ...
            clap::builder::PossibleValue::new("medium").help("Line number and notes"),
            clap::builder::PossibleValue::new("short").help("One line per diagnostic"),
        ]))
        .default_value("rich")
    ...
    match matches.subcommand() {
        Some(("compile", sub)) => {
            ...
            let style = parse_display_style(sub.get_one::<String>("diagnostics").unwrap());
        ...
        Some(("format", sub)) => cmd_format(sub.get_one::<String>("path").unwrap()),
        _ => unreachable!(),
    }
}
...
fn emit_diagnostics(
    ...
    for d in diagnostics {
        ...
        term::emit_to_write_style(&mut writer.lock(), &config, &compilation.files, d).unwrap();
    }
    has_errors
}
...
fn cmd_compile(file_path: &str, style: DisplayStyle) {
    ...
    for crate_graph in &compilation.crates {
        if emit_diagnostics(&compilation, &crate_graph.diagnostics, style.clone()) {
            std::process::exit(1);
        }
    }
...
fn cmd_format(file_path: &str) {
    ...
    let parsed = ast::Parser::parse(file_id, &files, &mut interner, &mut id_gen);
    let source = &files.get(file_id).unwrap().source;
    let formatted = fmt::format(&parsed, &interner, source, fmt::RendererConfig::default());
