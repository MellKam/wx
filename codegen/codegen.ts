...
import { TokenKind, TokenName } from "../parser/lexer.ts";
import {
	StatementKind,
...
const codegen_expression = (
	...
	if (expression.kind === ExpressionKind.Identifier) {
		...
		return [[OpCode.LocalGet, ...encodeULEB128(local.index)]];
	}
	if (expression.kind === ExpressionKind.Assignment) {
		const { id: left, value: right } = expression as AssignmentExpression;
		const local = locals.get(left.value);
		if (!local) {
			throw new Error("Invalid identifier");
		}
		const instructions = codegen_expression(locals, right);
		return [...instructions, [OpCode.LocalSet, ...encodeULEB128(local.index)]];
	}
	if (expression.kind === ExpressionKind.Unary) {
		...
		if (operator === TokenKind.Dash) {
			return [
				...instructions,
				[OpCode.I32Const, ...encodeULEB128(1)],
				[OpCode.I32Xor],
			];
		}
		throw new Error(`Invalid unary operator "${TokenName[operator]}"`);
	}
	if (expression.kind === ExpressionKind.Int) {
		const { value } = expression as IntExpression;
		return [[OpCode.I32Const, ...encodeULEB128(value)]];
	}
	if (expression.kind === ExpressionKind.Float) {
		const { value } = expression as FloatExpression;
		return [[OpCode.F32Const, ...encodeULEB128(value)]];
	}
	if (expression.kind === ExpressionKind.Binary) {
		...
		if (operator === TokenKind.Star) {
			return [...left_instructions, ...right_instructions, [OpCode.I32Mul]];
		}
...
const codegen_statement = (
	...
	switch (statement.kind) {
		case StatementKind.Expression: {
			const { value: expression } = statement as ExpressionStatement;
			return codegen_expression(locals, expression);
		}
		case StatementKind.ReturnValue: {
			const { value } = statement as ReturnValueStatement;
		...
		case StatementKind.VariableDefinition: {
			...
			const instructions: Instruction[] = [
				...
				[OpCode.LocalSet, ...encodeULEB128(locals.size)],
			];
...
export const codegen_function = (func: FunctionDefinitionStatement) => {
	...
	const instructions = func.body.body.flatMap((statement) =>
